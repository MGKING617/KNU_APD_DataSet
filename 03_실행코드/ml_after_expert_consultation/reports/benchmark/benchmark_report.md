...
- 임계값 선택 정책: 전문가 상담 후 비교 실험에서는 기존 LightGBM 기준 임계값을 반올림한 0.59를 모든 모델 평가에 고정 적용했습니다.
...
| ------------------- | ----- | ---- | --- | --- | --------- |
| Logistic Regression | 19953 | 5888 | 226 | 868 | 0.590     |
| XGBoost             | 20087 | 5754 | 241 | 853 | 0.590     |
| LightGBM            | 20345 | 5496 | 276 | 818 | 0.590     |
| Random Forest       | 23274 | 2567 | 636 | 458 | 0.590     |
...
| ------------------- | ------------- | ------------- | ------------------ | --------------- | ------------- | ------------- |
| LightGBM            | 0.835 ± 0.006 | 0.197 ± 0.004 | 0.129 ± 0.002      | 0.783 ± 0.019   | 0.222 ± 0.003 | 0.777 ± 0.006 |
| Logistic Regression | 0.840 ± 0.007 | 0.197 ± 0.009 | 0.129 ± 0.002      | 0.802 ± 0.017   | 0.222 ± 0.004 | 0.772 ± 0.003 |
| Random Forest       | 0.801 ± 0.003 | 0.137 ± 0.004 | 0.147 ± 0.004      | 0.415 ± 0.010   | 0.217 ± 0.006 | 0.878 ± 0.002 |
| XGBoost             | 0.839 ± 0.006 | 0.200 ± 0.004 | 0.128 ± 0.002      | 0.802 ± 0.016   | 0.221 ± 0.003 | 0.770 ± 0.005 |
...
- Recall 기준 상위 모델: **Logistic Regression** (0.793)
- PR-AUC 기준 상위 모델: **XGBoost** (0.198)
- F1 기준 상위 모델: **Random Forest** (0.222)

다만 이 결과는 현재 XLSX 원자료와 현재 변수 조합 안에서의 비교이므로, 외부 데이터 검증이나 추가 원자료가 들어오면 다시 비교해야 합니다.

웹앱에 현재 연결된 LightGBM은 recall 기준 1위는 아니지만, positive recall 0.748, PR-AUC 0.196로 비교 후보들과 함께 검토할 수 있습니다. 따라서 발표에서는 '현재 운영 모델은 LightGBM이며, 조기 선별 목적에 맞춰 recall 중심 임계값을 사용했다'고 설명하는 편이 가장 정직합니다.
